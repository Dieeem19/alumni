...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (userName) {
    const isAlumni = window.location.pathname.toLowerCase().includes('alumni');
    let topName = userName;
    if (isAlumni) {
      if (userName.includes(',')) {
        topName = userName.split(',')[0].trim();
      } else {
        const parts = userName.trim().split(/\s+/);
        topName = parts.length > 1 ? parts[parts.length - 1] : parts[0];
      }
    }
    const topbarUserSpan = document.querySelector('.topbar-user span');
    if (topbarUserSpan) topbarUserSpan.textContent = topName;
